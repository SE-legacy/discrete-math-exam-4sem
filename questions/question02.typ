...
/ Перестановками из $n$ элементов: называются все возможные последовательности из этих $n$ элементов, отличающиеся друг от друга порядком следования. Другими словами, когда мы говорим о перестановках, мы выкладываем в ряд $n$ элементов и получаем перестановку. Меняем порядок --- получаем новую перестановку.

/ Перестановки без повторений: Если среди $n$ элементов нет повторяющихся, тогда число перестановок без повторений равно

$ P_n = n! $

/ Перестановки с повторениями: Пусть будет $k$ типов объектов. \ Первый тип повторяется $n_1$ раз, второй $n_2$ раз, ..., $k$-й тип повторяется $n_k$ раз. \ Общее количество объектов $n = n_1 + n_2 + ... + n_k$. \ Тогда количество перестановок с повторениями равно
...
- Количество одинаковых перестановок объектов первого типа равно $n_1 !$, второго $n_2 !$, ..., \ $k$-го типа $n_k !$. Общее количество одинаковых перестановок равно $n_1 ! n_2 ! ... n_k !$.
...
/ Сочетанием без повторений из $n$ различных элементов по $k$: называются комбинации из $k$ элементов выбранных из исходных $n$ элементов, которые отличаются друг от друга только составом элементов.
...
Сочетания часто называют *биномиальными коэффициентами*, связывая их с биномом Ньютона. Вспомним:
...
/ Сочетания с повторениями из $n$ типов по $k$ элементов: называются все такие комбинации $k$ элементов из $n$ исходных типов, которые отличаются друг от друга составом элементов.
...
$ underbrace(1 space 1 ... 1, #[кол-во \ э-в \ 1 типа]) | underbrace(1 space 1... 1, #[2 типа]) | dots | underbrace(1 space 1 ... 1, #[$n$-й тип]) $
