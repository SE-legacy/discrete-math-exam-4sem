...
=== Пример
$
underbrace(1, a_0), underbrace(0, a_1), underbrace(1, a_2), 1, 1, dots; quad A(t) = 1 + t^2 + t^3 + dots
$

$
1, -1, 1, -1, dots; quad A(t) = 1 - t + t^2 - t^3 = limits(sum)_(n = 0) (- 1)^n t^n
$

== Сумма производящих функций
/ Суммой производящих функций: $A(t) = limits(sum)_(n = 0)^infinity a_n t^n$ и $B(t) = limits(sum)_(n = 0)^infinity b_n t^n$ назовем производящую функцию

$
C(t) = A(t) + B(t) = c_0 + c_1 t + c_2 t^2 + dots,
$
...
/ Подстановкой в производящую функцию: $A(t) = limits(sum)_(n = 0)^infinity a_n t^n$ производящей функции $B(t) = limits(sum)_(n = 0)^infinity b_n t^n$, $B(0) = b_0 = 0$ называется производящая функция 
