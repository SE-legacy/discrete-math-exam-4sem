...
+ Любые две вершины графа $G$ соединены единственной цепью.
+ $G$ --- связный граф и число вершин в нем на единицу больше числа ребер $n = m +1$.
+ $G$ не содержит циклов и $n = m + 1$.

*Доказательство*:

_Необходимость_: Мы знаем, что $G$ --- дерево, докажем 3 пункта.
+ Покажем, что любые две вершины графа $G$ соединены одной цепью. Пойдем от противного: пусть существуют 2 цепи, соединяющие 2 вершины графа $=>$ существует цикл $=>$ граф не является деревом. Получили противоречие.
...
_Достаточность_: Покажем, что из каждого из 3 условий следует, что граф является деревом.

+ Пойдем от противного: пусть $G$ не является деревом $=>$ хотя бы один цикл $=>$ найдутся 2 вершины, которые соединены более чем одной цепью. Противоречие.
+ Тоже от противного: пусть $G$ не является деревом $=>$ хотя бы один цикл. Удалим все висячие вершины (вершины степени 1), тогда получим граф $G'$ без висячих вершин, где $n' = m' + 1$. Посчитаем $n$: по лемме о рукопожатиях $2 n' lt.eq limits(sum)^n_(i = 1) d(v_1) = 2m' space => space n' lt.eq 'm$, что означает противоречие.
